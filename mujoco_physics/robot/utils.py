...
# --- 1. Math Helper Functions (Rodrigues & Exponential Map) ---
def skew(v):
...
def vec_to_se3(S, theta):
    """Screw Axis S=(w, v)와 각도 theta를 받아 SE(3) 변환 행렬 반환"""
    w = S[:3]
    v = S[3:]
    
    # Rodrigues' Formula
    w_skew = skew(w)
    R = np.eye(3) + np.sin(theta) * w_skew + (1 - np.cos(theta)) * np.dot(w_skew, w_skew)
    
    # G(theta) * v (Translation component)
    G = np.eye(3) * theta + (1 - np.cos(theta)) * w_skew + (theta - np.sin(theta)) * np.dot(w_skew, w_skew)
    p = np.dot(G, v)
